...
uses
  ...
  JvExControls, JvTimeLine, dxSkinsCore, dxSkinOffice2010Black,
...
type
  TfrmMMBondList = class(TForm)
    ...
    procedure jvCouponItemClick(Sender: TObject; Item: TJvTimeItem);
    procedure grdAllocationsClick(Sender: TObject);
...
begin
    ...
    and not bRefreshing and spMMBondList.Active then begin
        with spMMBondCouponList do begin
            ...
            jvCoupon.Items.Clear;
            
            for i := 1 to RecordCount do begin
                j := jvCoupon.Items.Add;
                j.Caption := 'Period ' + spMMBondCouponListPeriod.AsString;
                j.Date := spMMBondCouponListStartDate.Value;
                j.WidthAs := asDays;
                j.Width := spMMBondCouponListTenor.Value;
                j.Color := clYellow;
                FindNext;
            end;
        ...
        jvCoupon.FirstVisibleDate := dtmMain.CurrentWorkDate;
        jvCoupon.Refresh;
...
procedure TfrmMMBondList.jvCouponItemClick(Sender: TObject;
  Item: TJvTimeItem);
begin
    spMMBondCouponList.Locate('Period', Item.Index + 1, []);
end;

procedure TfrmMMBondList.grdAllocationsClick(Sender: TObject);
begin
    if jvCoupon.Selected <> nil then
        jvCoupon.Selected.Selected := False;

    jvCoupon.Items[spMMBondCouponListPeriod.Value - 1].Selected := True;
    jvCoupon.FirstVisibleDate := spMMBondCouponListStartDate.Value;
end;
